...
#ifdef DUST
#include "../data/dust_particle_data.h"
#endif
#include "../main/main.h"
...
class simparticles : public intposconvert, public setcomm
{
 public:
  ...
  #ifdef DUST
    dust_data *DustP;  // Dust particle properties
  #endif
  ...
  void allocate_memory(void)
  {
    ...
    #ifdef DUST
      DustP = (dust_data *)Mem.mymalloc_movable_clear(&DustP, "DustP", MaxPart * sizeof(dust_data));
    #endif
  ...
  void reallocate_memory_maxpart(int maxpartNew)
  {
    ...
    #ifdef DUST
    DustP = (dust_data *)Mem.myrealloc_movable(DustP, maxpartNew * sizeof(dust_data));
    if(maxpartNew > MaxPart)
      memset(((char *)DustP) + MaxPart * sizeof(dust_data), 0, (maxpartNew - MaxPart) * sizeof(dust_data));
    #endif
...
// This is an effort to address negative densities that sometimes arise due to numerical issues.
// It adds a safety check before updating thermodynamic variables.
inline void set_thermodynamic_variables_safe(simparticles* Sp, int i)
{
  if (Sp->P[i].getType() != 0) return;
  
  if(!std::isfinite(Sp->SphP[i].Density) || Sp->SphP[i].Density <= 0)  // ✅ NEW
  {
    printf("WARNING[T=%d]: Bad density for particle %lld: rho=%g\n",
           All.ThisTask, (long long)Sp->P[i].ID.get(), Sp->SphP[i].Density);
    return;
  }
