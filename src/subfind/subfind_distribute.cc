      ...
      if(target != CommThisTask)
        {
          ...
          #ifdef DUST
          // Handle DustP when moving last particle to slot n
          if(Tp->P[Tp->NumPart - 1].getType() == 6) {
            Tp->DustP[n] = Tp->DustP[Tp->NumPart - 1];  // Copy dust data
          } else {
            memset(&Tp->DustP[n], 0, sizeof(dust_data));  // Zero non-dust
          }
          #endif
