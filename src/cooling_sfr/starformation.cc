...
// Always compute and save stellar evolution stats (regardless of debug level)
if(tot_stars_spawned > 0 || tot_stars_converted > 0)
{
    ...
    double total_dm_mass = 0;
    MPI_Allreduce(&local_dm_mass, &total_dm_mass, 1, MPI_DOUBLE, MPI_SUM, Communicator);

    // Convert to physical units
    double stellar_to_halo = (total_dm_mass > 0) ? total_stellar_mass / total_dm_mass : 0.0;
    double stellar_mass_msun = total_stellar_mass * (All.UnitMass_in_g / SOLAR_MASS);
    double dm_mass_msun = total_dm_mass * (All.UnitMass_in_g / SOLAR_MASS);

    /* ALWAYS write to file (independent of debug level)
    if(ThisTask == 0)
    {
        char fname[512];
