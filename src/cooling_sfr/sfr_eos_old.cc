...
void coolsfr::cooling_and_starformation(simparticles *Sp)
{
  ...
  static int __sfr_eos_banner = 0;
  if(!__sfr_eos_banner && ThisTask==0){
    mpi_printf("[SFR-EOS] cooling_and_starformation() is ACTIVE (compiled from %s, %s)\n",
...
void coolsfr::set_units_sfr(void)
{
  ...
  if(ThisTask==0)
    mpi_printf("[SFR-EOS] set_units_sfr(): PhysDensThresh=%g (code), OverDensThresh=%g\n",
              All.PhysDensThresh, All.OverDensThresh);
}
#endif
