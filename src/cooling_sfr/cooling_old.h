...
#define NCOOLTAB 2000
...
#ifdef STARFORMATION
  /* metal cooling table structure */
  struct metal_cooling_table
  {
    std::vector<double> metallicity_bins;  // log(Z/Z_solar)
    std::vector<double> temperature_bins;  // log(T)
    std::vector<std::vector<double>> cooling_rates;  // Lambda(Z,T) in erg cm^3 s^-1
    int n_metallicity;
    int n_temperature;
    double z_min, z_max;  // log(Z/Z_solar) range
    double t_min, t_max;  // log(T) range
    bool table_loaded;
  };
#endif
...
#ifdef STARFORMATION
  metal_cooling_table metal_table;  /**< metal line cooling rates */
#endif
...
#ifdef COOLING
  double DoCooling(double u_old, double rho, double dt, double *ne_guess, gas_state *gs, do_cool_data *DoCool, double metallicity = 0.0);
  double GetCoolingTime(double u_old, double rho, double *ne_guess, gas_state *gs, do_cool_data *DoCool, double metallicity = 0.0);
  ...
  double CoolingRate(double logT, double rho, double *nelec, gas_state *gs, const do_cool_data *DoCool, double metallicity = 0.0);
  double CoolingRateFromU(double u, double rho, double *ne_guess, gas_state *gs, const do_cool_data *DoCool, double metallicity = 0.0);
...
#ifdef STARFORMATION
  /* Metal cooling helper functions */
