...
/*! \brief Print distribution of particles across hydro timebins for diagnostics
 *
 * This helps identify if feedback is creating particles with extremely small timesteps
 */
void sim::print_timestep_distribution(void)
{
  ...
  for(int i = 0; i < Sp.NumGas; i++)
  {
    ...
    {
      int bin = Sp.P[i].getTimeBinHydro();
  ...
  for(int b = 0; b < TIMEBINS; b++)
  {
    if(bin_counts[b] > 0)
    {
      // Calculate actual timestep for this bin
      double dt = (b > 0) ? (((integertime)1) << b) * All.Timebase_interval : 0;
      mpi_printf(" [%d:%.2e=%d]", b, dt, bin_counts[b]);
    }
  }
  mpi_printf("\n");
}