...
void sim::find_hydro_timesteps(void)
{
  ...
  // Update CourantFac based on redshift
    ...
    if(z > 20.0)
      ...
      All.CourantFac = All.CourantFac_z10;
    else if(z > 2.0)
      All.CourantFac = All.CourantFac_z2;
    else
      All.CourantFac = All.CourantFac_z0;
  // end of adaptive Courant factor
  ...
  for(int p = 0; p < NumPart; p++)
    {
      ...
      // Skip dust from PM timestep constraint:
      // without this, dust being on leads to gadget skipping snapshot outputs
      #ifdef DUST
            if(P[p].getType() == 6)
              continue;
      #endif
