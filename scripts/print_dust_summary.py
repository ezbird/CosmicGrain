...
def read_snapshot(snap_path):
    ...
    Handles both:
      - snapdir_049/ (directory with snapshot_049.*.hdf5 inside)
      - snapshot_049 (base name, looks for snapshot_049.*.hdf5)
    """
    # Determine if input is a directory or base name
    if os.path.isdir(snap_path):
        # It's a directory like snapdir_049/
        snap_num = snap_path.rstrip('/').split('_')[-1]
        pattern = os.path.join(snap_path, f"snapshot_{snap_num}.*.hdf5")
    else:
        # It's a base name like snapshot_049 or snapdir_049/snapshot_049
        ...
        with h5py.File(f, 'r') as hf:
            ...
            if 'PartType6' in hf:
                ...
                if n_dust > 0:
                    ...
                    print(f"  File {os.path.basename(f)}: {n_dust:,} dust particles")
    ...
    for key in data:
        if isinstance(data[key], list) and len(data[key]) > 0:
            ...
            if key == 'dust_pos':
                print(f"Total dust particles loaded: {len(data[key]):,}")
            elif key == 'gas_pos':
                print(f"Total gas particles loaded: {len(data[key]):,}")
...
def find_nearest_gas(dust_pos, gas_pos, gas_vel, max_search=10.0):
    ...
    print(f"  Finding nearest neighbors (max distance {max_search} kpc)...")
...
def analyze_snapshot(snap_path, halo_center=None):
    ...
    for i in range(len(bins)-1):
        ...
        print(f"   [{bins[i]:3.0f}-{bins[i+1]:3.0f} nm]: {counts[i]:6d} grains ({frac_num:5.1f}%), {masses[i]:.2e} Msun ({frac_mass:5.1f}%)")
    print()
    
    # Radial distribution
    if halo_center is None:
        # Use box center as fallback
    ...
    for i in range(len(bins)-1):
        frac_num = 100 * counts[i] / total_count if total_count > 0 else 0
        frac_mass = 100 * masses[i] / total_mass if total_mass > 0 else 0
        print(f"   [{bins[i]:4.0f}-{bins[i+1]:4.0f} kpc]: {counts[i]:6d} grains ({frac_num:5.1f}%), {masses[i]:.2e} Msun ({frac_mass:5.1f}%)")
    ...
    if len(data['gas_pos']) > 0:
        ...
        if gas_vel_nearest is not None:
            ...
            for i in range(len(rad_bins)-1):
                ...
                if np.sum(mask) > 0:
                    median_dv = np.median(dv_mag[mask])
                    print(f"     {rad_bins[i]:4d}-{rad_bins[i+1]:4d} kpc: Δv = {median_dv:5.1f} km/s (n={np.sum(mask):,})")
    else:
    ...
    if len(data.get('dust_carbon', [])) > 0:
        ...
        print(f"   Silicate grains (CF<0.5): {np.sum(sil_mask):,} ({100*np.sum(sil_mask)/len(carb):.1f}%)")
        print(f"   Carbon grains (CF≥0.5): {np.sum(carb_mask):,} ({100*np.sum(carb_mask)/len(carb):.1f}%)")
        print()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Analyze dust from GADGET snapshots',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Analyze single snapshot directory
  python analyze_dust_snapshots.py snapdir_017/ --center 23937.27 24238.31 24548.18
  
  # Analyze multiple epochs
  python analyze_dust_snapshots.py snapdir_010/ snapdir_013/ snapdir_017/ --center 23937.27 24238.31 24548.18
  
  # Use base name (if files are not in directories)
  python analyze_dust_snapshots.py snapshot_017 --center 23937.27 24238.31 24548.18
        """
    )
    parser.add_argument('snapshots', nargs='+', help='Snapshot directories (e.g., snapdir_017/) or base names')
    ...
    if halo_center is not None:
        print(f"Using halo center: ({halo_center[0]:.2f}, {halo_center[1]:.2f}, {halo_center[2]:.2f}) kpc")
