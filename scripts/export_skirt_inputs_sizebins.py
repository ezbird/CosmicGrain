...
DEFAULT_N_SIZE_BINS = 12
DEFAULT_MIN_GRAIN_NM = 1.0
DEFAULT_MAX_GRAIN_NM = 200.0
# ─────────────────────────────────────────────────────────────────────────
...
def export_size_binned_dust(dust_path, out_dir, n_bins=DEFAULT_N_SIZE_BINS,
                            min_grain_nm=DEFAULT_MIN_GRAIN_NM,
                            max_grain_nm=DEFAULT_MAX_GRAIN_NM,
                            label=""):
    """
    Bin dust by actual PartType6/GrainRadius and split every radius bin into
    silicate and carbon SKIRT ParticleMedium files.

    Output particle files contain only:
        x_pc  y_pc  z_pc  h_pc  dust_mass_Msun

    The size bin itself is recorded in dust_size_bins*.txt and should be
    represented by the corresponding narrow grain population in the .ski file.
    """
    data = np.loadtxt(dust_path)

    if data.size == 0:
        print(f"  Size bins {Path(dust_path).name}: empty — skipping")
        return []
    ...
    if data.shape[1] < 7:
        raise ValueError(
            f"{dust_path} needs >=7 columns: x y z h M f_carbon GrainRadius_nm"
        )

    x, y, z, hh, mass, fc, grain_nm = data[:, :7].T

    valid = (
        np.isfinite(grain_nm)
        & np.isfinite(mass)
        & np.isfinite(fc)
        & (grain_nm > 0)
        & (mass > 0)
    ...
    if not np.all(valid):
        print(f"  WARNING: dropping {(~valid).sum()} invalid dust rows")
        x, y, z, hh, mass, fc, grain_nm = [
            arr[valid] for arr in (x, y, z, hh, mass, fc, grain_nm)
        ]

    below = grain_nm < min_grain_nm
    above = grain_nm > max_grain_nm
    if below.any() or above.any():
        print(
            f"  WARNING: GrainRadius outside [{min_grain_nm:g}, {max_grain_nm:g}] nm: "
            f"{below.sum()} below, {above.sum()} above. "
            "Radii are clipped only for bin assignment."
        )

    grain_for_bin = np.clip(grain_nm, min_grain_nm, max_grain_nm)

    # Logarithmic bins are appropriate across the wide 1–200 nm range.
    edges = np.geomspace(min_grain_nm, max_grain_nm, n_bins + 1)
    bin_id = np.digitize(grain_for_bin, edges[1:-1], right=False)
    ...
    print(
        f"  Size-binning {Path(dust_path).name}: {len(mass)} particles into "
        f"{n_bins} logarithmic bins over {min_grain_nm:g}–{max_grain_nm:g} nm"
    )
    ...
    for i in range(n_bins):
        ...
        lo = edges[i]
        hi = edges[i + 1]
        representative = np.sqrt(lo * hi)

        if not np.any(use):
            manifest_rows.append((i, lo, hi, representative, 0, 0.0, 0.0))
            print(f"    bin {i:02d}: {lo:7.3f}–{hi:7.3f} nm : empty")
            continue

        f = np.clip(fc[use], 0.0, 1.0)
        m = mass[use]

        m_sil = m * (1.0 - f)
        m_car = m * f

        base = np.column_stack([x[use], y[use], z[use], hh[use]])

        m_sil_total = m_sil.sum()
        m_car_total = m_car.sum()
        total_out += m_sil_total + m_car_total

        sil_path = out_dir / f"dust_silicate{label}_bin{i:02d}.txt"
        car_path = out_dir / f"dust_carbon{label}_bin{i:02d}.txt"

        if m_sil_total > 0:
            keep = m_sil > 0
            write_skirt_table(
                sil_path,
                np.column_stack([base[keep], m_sil[keep]]),
                columns5,
            )
            written.append(sil_path)

        if m_car_total > 0:
            keep = m_car > 0
            write_skirt_table(
                car_path,
                np.column_stack([base[keep], m_car[keep]]),
                columns5,
            )
            written.append(car_path)

        manifest_rows.append(
            (i, lo, hi, representative, int(use.sum()), m_sil_total, m_car_total)
        )

        print(
            f"    bin {i:02d}: {lo:7.3f}–{hi:7.3f} nm "
            f"(rep={representative:7.3f}) : {use.sum():4d} particles, "
            f"M_sil={m_sil_total:.3e}, M_car={m_car_total:.3e} Msun"
        )

    manifest = out_dir / f"dust_size_bins{label}.txt"
    with open(manifest, "w") as f:
        f.write("# bin min_nm max_nm representative_nm N_particles M_sil_Msun M_car_Msun\n")
        for row in manifest_rows:
            f.write(
                f"{row[0]:02d} {row[1]:.8g} {row[2]:.8g} {row[3]:.8g} "
                f"{row[4]:d} {row[5]:.8e} {row[6]:.8e}\n"
            )

    relerr = abs(total_out - total_in) / max(total_in, 1e-30)
    print(
        f"    Size-bin mass check: input={total_in:.6e}, "
        f"output={total_out:.6e} Msun, rel.err={relerr:.3e}"
    )
...
def process_snapshot(sim_dir, snap_num, aperture_pkpc, out_root, include_lrn=True,
                     snap_num_z0=None, n_size_bins=DEFAULT_N_SIZE_BINS,
                     min_grain_nm=DEFAULT_MIN_GRAIN_NM,
                     max_grain_nm=DEFAULT_MAX_GRAIN_NM,
                     aperture_r200=False):
    ...
    # Resolve the selection aperture independently for every snapshot.
    # This is important for redshift series because R200 evolves with time.
    if aperture_r200:
    ...
    if result is not None:
        ...
        # Preserve the actual evolved PartType6 GrainRadius distribution.
        export_size_binned_dust(
            dust_path, out_dir,
            n_bins=n_size_bins,
            min_grain_nm=min_grain_nm,
            max_grain_nm=max_grain_nm,
        )
...
def main():
    ...
    parser.add_argument("--aperture-r200", action="store_true",
                         help="Use the snapshot-specific R200 returned by halo_utils as the "
                              "selection aperture. Overrides --aperture. Recommended when "
                              "exporting multiple redshifts.")
    ...
    parser.add_argument("--size-bins", type=int, default=DEFAULT_N_SIZE_BINS,
                          help=f"Number of logarithmic PartType6 GrainRadius bins "
                               f"(default {DEFAULT_N_SIZE_BINS})")
    parser.add_argument("--min-grain-nm", type=float, default=DEFAULT_MIN_GRAIN_NM,
                          help=f"Minimum radius of SKIRT grain-size grid in nm "
                               f"(default {DEFAULT_MIN_GRAIN_NM})")
    parser.add_argument("--max-grain-nm", type=float, default=DEFAULT_MAX_GRAIN_NM,
                          help=f"Maximum radius of SKIRT grain-size grid in nm "
                               f"(default {DEFAULT_MAX_GRAIN_NM})")
    ...
    for snap_num in range(args.snap_start, snap_end + 1):
        process_snapshot(
            ...
            min_grain_nm=args.min_grain_nm,
            max_grain_nm=args.max_grain_nm,
